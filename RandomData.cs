...
namespace uti
{
	public static class RandomData
	{
		...
		public static string GetProductName()
		{
			...
			//	IEnumerable<String> NounIEnumerableList = File.ReadLines(System.Environment.CurrentDirectory + "\\Nouns.txt");
			...
			var rand = new Random(Convert.ToInt16(Math.Truncate(RandomData.GetNormal(160, 33))));
			...
			for (int s = 0; s < 1; s++)
			{
				...
				for (int w = 0; w < 3; w++)
				{
					...
					result.Append(ProductType[rand.Next(ProductType.Length)]);
				}
		...
		public static Int32 GetRandNum()
		{
			Int32 x; // int32 rand;
			x = Convert.ToInt16(Math.Truncate(RandomData.GetNormal(160, 33)));
			return x;
